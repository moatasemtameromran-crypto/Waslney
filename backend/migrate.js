...
module.exports = async function runMigrations() {
  // MySQL-safe "add column if missing" (works on Oracle MySQL AND MariaDB).
  // MySQL does NOT support `ADD COLUMN IF NOT EXISTS`, so check first.
  async function addColumnIfMissing(table, column, definition) {
    try {
      ...
      console.warn(`⚠️  Could not add ${column} to ${table}:`, e.message);
    }
  }
  ...
  try {
    ...
    // 6. user columns (MySQL-safe — these are required by auth/admin/users routes)
    await addColumnIfMissing('users', 'email',          'VARCHAR(150) DEFAULT NULL');
    await addColumnIfMissing('users', 'profile_photo',  'LONGTEXT DEFAULT NULL');
    await addColumnIfMissing('users', 'account_status', "VARCHAR(30) NOT NULL DEFAULT 'active'");
    await addColumnIfMissing('users', 'rejection_note', 'TEXT DEFAULT NULL');
