...
import {
  ...
  Lightbulb, TicketCheck, Plus, UserPlus, ShieldCheck, Building2, ClipboardList,
} from "lucide-react";
...
const navItems: NavItem[] = [
  ...
  { label: "Trips", icon: MapPin, href: "/trips" },
  {
    ...
    children: [
      ...
      { label: "Tenders", href: "/tenders", icon: Building2 },
      { label: "Driver Review", href: "/review", icon: ShieldCheck },
      { label: "New Account", href: "/new-account", icon: UserPlus },
      { label: "Manage Bookings", href: "/manage-bookings", icon: Calendar },
    ],
  },
...
function NavLink({ item, collapsed, onClose }: { item: NavItem; collapsed: boolean; onClose: () => void }) {
  ...
  if (item.children) {
    const anyActive = item.children.some(c => location.startsWith(c.href));
    return (
      <div>
        <button
