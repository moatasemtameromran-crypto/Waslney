...
router.put("/:id", requireAdminAuth, async (req, res) => {
  ...
  try {
    await db.query(
      ...
      [document_name, document_category, number_of_images, gallery_restricted !== undefined ? (gallery_restricted ? 1 : 0) : null, doc_required !== undefined ? (doc_required ? 1 : 0) : null, doc_no_required !== undefined ? (doc_no_required ? 1 : 0) : null, doc_expiry_required !== undefined ? (doc_expiry_required ? 1 : 0) : null, status, req.params.id]
    );
    res.json({ ok: true });
  } catch (err: any) {
    res.status(500).json({ error: "Server error" });
