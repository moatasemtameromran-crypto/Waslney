...
router.get("/stats", requireAdminAuth, async (req, res) => {
  try {
    ...
    const [recentBookings] = await db.query(
      `SELECT b.id, b.status, b.created_at, b.seats,
              u.name AS passenger_name,
              t.from_loc AS origin, t.to_loc AS destination, t.price
       FROM bookings b
    ...
    const [recentTrips] = await db.query(
      `SELECT t.id, t.from_loc AS origin, t.to_loc AS destination,
              t.pickup_time AS departure_time, t.status, t.price,
