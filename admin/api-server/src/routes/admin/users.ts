...
router.get("/", requireAdminAuth, async (req, res) => {
  try {
    ...
    let query = `SELECT id, name, phone, email, role, account_status, car, plate,
...
router.get("/customers", requireAdminAuth, async (req, res) => {
  try {
    ...
    req.log.error({ err: err.message }, "Get customers error");
    res.status(500).json({ error: "Server error" });
  }
});
...
router.get("/drivers", requireAdminAuth, async (req, res) => {
  try {
    ...
    let query = `SELECT u.id, u.name, u.phone, u.email, u.car, u.plate,
                        u.profile_photo, u.account_status, u.rejection_note, u.created_at,
    ...
    req.log.error({ err: err.message }, "Get drivers error");
    res.status(500).json({ error: "Server error" });
  }
});

// GET /api/admin/users/delete-requests/list
router.get("/delete-requests/list", requireAdminAuth, async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT dar.id, dar.user_id, dar.reason, dar.status, dar.created_at,
              u.name, u.phone, u.email
       FROM delete_account_requests dar
...
// PUT /api/admin/users/delete-requests/:id
router.put("/delete-requests/:id", requireAdminAuth, async (req, res) => {
  const { status } = req.body;
  try {
    await db.query(
      "UPDATE delete_account_requests SET status = ? WHERE id = ?",
...
router.get("/:id", requireAdminAuth, async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT id, name, phone, email, role, account_status, car, plate,
              profile_photo, created_at FROM users WHERE id = ?`,
      [req.params.id]
    ) as any;
    if (!rows.length) { res.status(404).json({ error: "User not found" }); return; }

    const [userTrips] = await db.query(
      `SELECT t.id, t.from_loc AS origin, t.to_loc AS destination, t.pickup_time AS departure_time, t.status, t.price
    ...
    const [userBookings] = await db.query(
      `SELECT b.id, b.seats, b.status, b.created_at,
              t.from_loc AS origin, t.to_loc AS destination, t.pickup_time AS departure_time, t.price
...
router.put("/:id/status", requireAdminAuth, async (req, res) => {
  ...
  try {
    await db.query(
      "UPDATE users SET account_status = ?, rejection_note = ? WHERE id = ?",
