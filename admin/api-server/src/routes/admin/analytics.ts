...
router.get("/bookings", requireAdminAuth, async (req, res) => {
  try {
    ...
    let query = `
      SELECT b.id AS booking_id, u.name AS user, d.name AS driver,
             t.status AS trip_status, t.id AS trip_id,
             t.from_loc AS origin, t.to_loc AS destination,
             b.created_at AS booking_time,
             t.price AS amount, b.status AS booking_status,
             b.seats
      FROM bookings b
    ...
    req.log.error({ err: err.message }, "Analytics bookings error");
    res.status(500).json({ error: "Server error" });
  }
});

// GET /api/admin/analytics/driver-trips
router.get("/driver-trips", requireAdminAuth, async (req, res) => {
  try {
    const { from_date, to_date } = req.query as any;
    let query = `
      SELECT t.id AS trip_id, u.name AS driver_name, u.phone AS driver_phone,
             t.from_loc AS origin, t.to_loc AS destination,
             t.pickup_time AS departure_time,
             t.status, t.price,
      ...
      WHERE 1=1`;
    const params: any[] = [];
    if (from_date) { query += " AND DATE(t.pickup_time) >= ?"; params.push(from_date); }
    if (to_date) { query += " AND DATE(t.pickup_time) <= ?"; params.push(to_date); }
    query += " GROUP BY t.id ORDER BY t.pickup_time DESC LIMIT 500";
    const [rows] = await db.query(query, params) as any;
    res.json(rows);
  } catch (err: any) {
    res.status(500).json({ error: "Server error" });
...
// GET /api/admin/analytics/stop-utilization
router.get("/stop-utilization", requireAdminAuth, async (req, res) => {
  try {
    const [rows] = await db.query(`
      ...
      GROUP BY ts.label, ts.type, t.from_loc, t.to_loc
