...
// GET /api/admin/settings?city=Cairo&category=general
...
// PUT /api/admin/settings — upsert multiple
router.put("/", requireAdminAuth, async (req, res) => {
  const { settings, city, category } = req.body; // settings: {key: value}
  try {
    await ensureTable();
    for (const [key, value] of Object.entries(settings || {})) {
      await db.query(
        `INSERT INTO admin_settings (setting_key, setting_value, city, category)
         VALUES (?, ?, ?, ?)
...
// GET /api/admin/settings/city — city-specific settings
